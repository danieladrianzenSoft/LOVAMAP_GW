...
import TtlDropZone from '../components/TtlDropZone';
import { graphApi } from '../api/client';
import { RdfGraph, RdfGraphNode, RdfOntologySummary } from '../models/rdfGraph';
import { parseTtlToGraph, buildSummaryFromQuads } from '../helpers/ttlParser';
...
const GraphExplorer: React.FC = () => {
	...
	useEffect(() => {
		const handleClickOutside = (e: MouseEvent) => {
			...
			// Ignore clicks inside the search bar or the portaled dropdown
	...
	// Position dropdown aligned to search bar, clamped within viewport
	useLayoutEffect(() => {
	...
	useEffect(() => {
		...
		if (hash && hash.startsWith('#ttl=')) {
			...
			decompressString(decodeURIComponent(encoded))
				...
				.catch(() => setError('Failed to decompress shared link data.'));
		}
	}, []);
	...
	const handleModeChange = (newMode: Mode) => {
		...
		if (newMode === 'ttl') setTtlFiles(new Map());
		if (newMode !== 'ttl') window.location.hash = '';
	};
	...
	const handleCopyShareLink = async () => {
		...
		try {
			...
			if (compressed.length > 32_000) {
				toast.error('TTL data too large for URL sharing (> ~32KB compressed).');
				return;
			}
	...
	return (
		<div className="relative w-full h-full overflow-hidden">
			...
			<div ref={toolbarRef} className="absolute top-0 left-0 right-0 z-50 px-4 py-3 bg-gray-50/90 dark:bg-gray-900/90 backdrop-blur-sm flex items-center gap-3 flex-wrap">
				<h1 className="text-xl font-bold text-gray-800 dark:text-white mr-2">Graph Explorer</h1>
				...
				{graph && (
					<div ref={searchRef}>
						<div ref={searchInputRef} className="flex items-center bg-white dark:bg-gray-800 rounded-lg px-2 py-1 border border-gray-200 dark:border-gray-700 w-72">
							...
							<input
								...
								className="bg-transparent text-xs text-gray-700 dark:text-gray-300 outline-none flex-1 placeholder-gray-400"
							/>
							{searchQuery && (
				...
				{showSearchDropdown && searchResults.length > 0 && createPortal(
					<div
						...
						className="bg-white dark:bg-gray-800 shadow-lg rounded-lg border border-gray-200 dark:border-gray-700 max-h-60 overflow-y-auto"
				...
				{mode === 'ttl' && ttlFiles.size > 0 && (
					<div className="flex gap-2">
						<button onClick={handleCopyShareLink} className="px-3 py-1.5 text-xs font-medium bg-kb-600 text-white rounded-md hover:bg-kb-700 transition-colors">
							Copy Share Link
						</button>
						<button onClick={handleClearFiles} className="px-3 py-1.5 text-xs font-medium bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
							Clear
						</button>
					</div>
				)}
				...
				{mode === 'ttl' && ttlFiles.size > 0 && (
					<div className="flex items-center gap-2 flex-wrap">
						...
						{Array.from(ttlFiles.keys()).map((name) => (
							<span key={name} className="text-xs bg-kb-100 dark:bg-kb-900/30 text-kb-700 dark:text-kb-300 px-2 py-0.5 rounded">
								{name}
							</span>
						))}
						<button
							onClick={() => document.getElementById('ttl-add-more')?.click()}
							className="text-xs text-kb-600 dark:text-kb-400 hover:underline"
						>
							+ Add more
						</button>
						<input
			...
			{/* Graph area — offset below toolbar */}
			<div className="absolute left-0 right-0 bottom-0" style={{ top: toolbarHeight }}>
				{loading && (
				...
				{/* Right sidebar: Node Details + Properties stacked */}
				{graph && (
					<div
						className="absolute right-4 z-40 w-72 flex flex-col gap-3"
						style={{ top: 16, bottom: 16, pointerEvents: 'none' }}
					>
						{/* Node Details (only when a node is selected) */}
						{selectedNode && (
							<div
								className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-lg rounded-lg p-4 overflow-y-auto min-h-0 flex-shrink"
								style={{ pointerEvents: 'auto' }}
							>
								<div className="flex justify-between items-center mb-2">
									<h2 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Node Details</h2>
									<button
								...
								<div className="text-xs text-gray-600 dark:text-gray-400 space-y-2">
									{selectedNode.group && (
										<span className="inline-block px-2 py-0.5 rounded-full text-white text-xs font-medium" style={{ backgroundColor: getGroupColor(selectedNode.group) }}>
											{selectedNode.group}
										</span>
									)}
									...
									{nodeConnections.length > 0 && (
										<div className="border-t border-gray-200 dark:border-gray-700 pt-2 mt-2">
											<p className="font-medium text-gray-700 dark:text-gray-300 mb-1">Connections ({nodeConnections.length})</p>
											<div className="space-y-1 max-h-48 overflow-y-auto">
												{nodeConnections.map((conn, i) => (
													...
													>
														<span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: getGroupColor(conn.targetNode.group) }} />
						...
						{/* Spacer pushes Properties to bottom */}
						<div className="flex-1 min-h-0" />

						{/* Properties panel (always at bottom) */}
